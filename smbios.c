...
static void smbios_build_type_29_table(void)
{
	...
	SMBIOS_TABLE_SET_STR(29, description,"corsair Electrical");
    SMBIOS_BUILD_TABLE_POST;
}
...
static void smbios_build_type_39_table(void)
{
	...
	SMBIOS_TABLE_SET_STR(39, device_name,"corsair PowerSupply");
    SMBIOS_BUILD_TABLE_POST;
}
...
static void smbios_build_type_22_table(void)
{
	...
	SMBIOS_TABLE_SET_STR(22, device_name,"corsair Battery");
    SMBIOS_BUILD_TABLE_POST;
}
...
static void smbios_build_type_0_table(void)
{
    ...
    SMBIOS_TABLE_SET_STR(0, bios_version_str, "1002");//李晓流 dds666 modify

    t->bios_starting_address_segment = cpu_to_le16(0xE800); /* from SeaBIOS */

    SMBIOS_TABLE_SET_STR(0, bios_release_date_str, "03/31/2023");//李晓流 dds666 modify
...
static void smbios_build_type_1_table(void)
{
    ...
    SMBIOS_TABLE_SET_STR(1, manufacturer_str, "ASUS"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(1, product_name_str, "System Product Name"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(1, version_str, "System Version"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(1, erial_number_str, "System Serial Number"); //李晓流 dds666 modify 
    if (qemu_uuid_set) {
    ...
    SMBIOS_TABLE_SET_STR(1, sku_number_str, "SKU"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(1, family_str, "To be filled by O.E.M."); //李晓流 dds666 modify 
...
static void smbios_build_type_2_table(void)
{
    ...
    SMBIOS_TABLE_SET_STR(2, manufacturer_str, "ASUSTeK COMPUTER INC."); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(2, product_str, "PRIME Z790-P WIFI"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(2, version_str, "Rev 1.xx"); //李晓流 dds666 modify 
                                      ...
                                      Error **errp)
{
    ...
    SMBIOS_TABLE_SET_STR(4, processor_version_str, "13th Gen Intel(R) Core(TM) i9"); //李晓流 dds666 modify 
    t->voltage = 0x8B;
    ...
    t->l1_cache_handle = cpu_to_le16(0x0020); /* N/A */ //李晓流 dds666 modify L1缓存抄写物理机
    t->l2_cache_handle = cpu_to_le16(0x0021); /* N/A */ //李晓流 dds666 modify L2缓存抄写物理机
    t->l3_cache_handle = cpu_to_le16(0x0022); /* N/A */ //李晓流 dds666 modify L3缓存抄写物理机
...
static void smbios_build_type_17_table(unsigned instance, uint64_t size)
{
    ...
    SMBIOS_TABLE_SET_STR(17, device_locator_str, "Controller0-DIMM1");   //李晓流 dds666 modify 内存设备位置
    SMBIOS_TABLE_SET_STR(17, bank_locator_str, "BANK 0");  //李晓流 dds666 modify 内存插槽位置
    t->memory_type = 0x1F; /* DDR5 */  //李晓流 dds666 modify ddr4类型
    t->type_detail = cpu_to_le16(0x80); /* test 0x80*/ //李晓流 dds666 modify 0x80代表 Synchronous
    t->speed = cpu_to_le16(5200); //李晓流 dds666 modify 3200mhz
    SMBIOS_TABLE_SET_STR(17, manufacturer_str, "Corsair"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(17, serial_number_str, "00000000"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(17, asset_tag_number_str, "9876543210"); //李晓流 dds666 modify 
    SMBIOS_TABLE_SET_STR(17, part_number_str, "CMH64GX5M2B5600C36"); //李晓流 dds666 modify 
    t->attributes = 1; /* test 1 */ //李晓流 dds666 modify 1代表 记不得了，你要测一下
    t->configured_clock_speed = t->speed; /* reuse value for max speed */
    t->minimum_voltage = cpu_to_le16(1100); /* Unknown */ //李晓流 dds666 modify 1.2v
    t->maximum_voltage = cpu_to_le16(1250); /* Unknown */ //李晓流 dds666 modify  1.35v
    t->configured_voltage = cpu_to_le16(1100); /* Unknown */ //李晓流 dds666 modify 1.2v
                         ...
                         const char *version)
{
    ...
    SMBIOS_SET_DEFAULT(smbios_type1.manufacturer, "ASUS"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(smbios_type1.product, "System Product Name"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(smbios_type1.version, "System Version"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(type2.manufacturer, "ASUS"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(type2.product, "PRIME Z790-P WIFI"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(type2.version, "Rev 1.xx"); //李晓流 dds666 modify
    ...
    SMBIOS_SET_DEFAULT(type4.sock_pfx, "LGA1700");
    SMBIOS_SET_DEFAULT(type4.manufacturer, "Intel(R) Corporation"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(type4.version, "13th Gen Intel(R) Core(TM) i9-13900K"); //李晓流 dds666 modify
    SMBIOS_SET_DEFAULT(type17.loc_pfx, "DIMM");
    SMBIOS_SET_DEFAULT(type17.manufacturer, "Corsair"); //李晓流 dds666 modify
...
unsigned cores_per_socket = machine_topo_get_cores_per_socket(ms);
    ...
    smbios_build_type_7_table(0, "L1 Cache", 0x180, cores_per_socket * 0x30, 0x4, 0x4, 0x9); // L1 Data: 48 KB, 12-way set-associative
    // L1 Instruction Cache: 32 KB per core
    smbios_build_type_7_table(1, "L1 Cache", 0x180, cores_per_socket * 0x20, 0x4, 0x3, 0x7); // L1 Instruction: 32 KB, 8-way set-associative
    // L2 Cache: 2 MB per core
    smbios_build_type_7_table(2, "L2 Cache", 0x181, cores_per_socket * 0x800, 0x5, 0x5, 0x10); // L2 Cache: 2MB per core, 16-way set-associative
    // L3 Cache: 36 MB shared across all cores
    smbios_build_type_7_table(3, "L3 Cache", 0x182, 0x9000, 0x6, 0x5, 0x14); // L3 Cache: 36MB shared, 20-way set-associative
...
#define GET_DIMM_SZ ((i < dimm_cnt - 1) ? mc->smbios_memory_device_size \
	...
	smbios_build_type_26_table(4,"corsair",0x63);//李晓流 dds666 added
	smbios_build_type_26_table(5,"corsair",0x64);//李晓流 dds666 added
	smbios_build_type_26_table(6,"corsair",0x6A);//李晓流 dds666 added
	smbios_build_type_26_table(7,"corsair",0x67);//李晓流 dds666 added
	...
	smbios_build_type_27_table(3,"corsair",0x65);//李晓流 dds666 added
	smbios_build_type_27_table(4,"corsair",0x63);//李晓流 dds666 added
	smbios_build_type_27_table(5,"corsair",0x67);//李晓流 dds666 added
	...
	smbios_build_type_28_table(3,"corsair",0x67);//李晓流 dds666 added
	smbios_build_type_28_table(4,"corsair",0x69);//李晓流 dds666 added
	smbios_build_type_28_table(5,"corsair",0x63);//李晓流 dds666 added
	smbios_build_type_28_table(6,"corsair",0x6A);//李晓流 dds666 added
